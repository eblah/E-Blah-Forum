...
EOT
  ...
  <td class="win2" style="width: 50%">$new<a href="$surl\lm-$id/$snew" title="$gtxt{'19'} $posted_sn">$title</a>$newend<div class="smalltext">$gtxt{'46'} $lastuser<div class="bidate">$date from the <a href="$surl\lb-$board" title="$mydesc">$mydesc</a></div></div></td>
EOT
		}
		++$counter;
		++$counter3;
